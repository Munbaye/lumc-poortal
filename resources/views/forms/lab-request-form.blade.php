...
Non-editable fields: Requesting Physician, footer timestamps (Date, Request Received,
Specimen Collected, Test Started, Test Done) — these are filled by the system or tech.
--}}
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Read-only field — same visual as .fi but non-interactive */
        .fi-ro {
            width: 100%;
            border: none;
            border-bottom: 1px solid #888;
...
<body>
    ...
    <div class="toolbar no-print">
        ...
        @isset($patient)<span class="tag"
        style="background:rgba(16,185,129,.22);border-color:rgba(16,185,129,.45);">{{ $patient->case_no }}</span>@endisset
        <span class="hint">Click tests to select · Submit saves to system · Print produces hardcopy</span>
        <span class="spacer"></span>
        <button id="btnSubmit" class="btn-submit" onclick="submitForm()"><x-heroicon-o-check style="width:14px;height:14px;display:inline-block;vertical-align:middle;margin-right:5px;" /> Submit Request</button>
        &nbsp;
        <button class="btn-print" onclick="window.print()"><x-heroicon-o-printer
                style="width:14px;height:14px;display:inline-block;vertical-align:middle;margin-right:5px;" />Print /
            PDF</button>
    </div>
    ...
    <div class="paper">
        ...
        <div class="header">
            ...
            <div class="header-center">
                ...
                <div class="h-ref">LAB-001-1 Rev. 1 &nbsp;·&nbsp; Brgy. Nazareno, Agoo, La Union &nbsp;·&nbsp; (072)
                    607-5541 loc. 117/118</div>
            </div>
        ...
        {{-- ── ROW 1: Date / Hospital No / Receipt No / Ward ─── --}}
        <div class="g4" style="margin-bottom:5px;">
            <div class="fg"><span class="fl">Date of Request</span><input type="date" id="f_date" class="fi"
                    value="{{ $today ?? '' }}"></div>
            <div class="fg"><span class="fl">Hospital No. (Case No.)</span><input type="text" id="f_hosp" class="fi"
                    value="{{ $hospitalNo ?? '' }}"></div>
            <div class="fg"><span class="fl">Receipt No.</span><input type="text" id="f_receipt" class="fi"
                    value="{{ $requestNo }}" style="font-family:monospace;font-weight:bold;"></div>
            <div class="fg"><span class="fl">Ward / Service</span><input type="text" id="f_ward" class="fi"
                    value="{{ $ward ?? '' }}"></div>
        </div>

        {{-- ── ROW 2: Name ─── --}}
        <div class="g4" style="margin-bottom:5px;">
            <div class="fg"><span class="fl">Surname</span><input type="text" id="f_family" class="fi"
                    value="{{ $familyName ?? '' }}"></div>
            <div class="fg"><span class="fl">First Name</span><input type="text" id="f_first" class="fi"
                    value="{{ $firstName ?? '' }}"></div>
            <div class="fg"><span class="fl">Middle Name</span><input type="text" id="f_middle" class="fi"
                    value="{{ $middleName ?? '' }}"></div>
            <div class="fg"><span class="fl">Address</span><input type="text" id="f_addr" class="fi"
                    value="{{ $address ?? '' }}"></div>
        </div>

        {{-- ── ROW 3: Demographics ─── --}}
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr 2fr;gap:8px;margin-bottom:5px;">
            <div class="fg"><span class="fl">Birth Date</span><input type="date" id="f_dob" class="fi"
                    value="{{ $dateOfBirth ?? '' }}"></div>
            <div class="fg"><span class="fl">Age</span><input type="text" id="f_age" class="fi"
                    value="{{ $age ?? '' }}"></div>
            <div class="fg">
            ...
            <div class="fg">
                ...
                <div style="display:flex;gap:8px;margin-top:3px;">
                    <label style="display:inline-flex;align-items:center;gap:3px;font-size:8.5pt;cursor:pointer;"><input
                            type="radio" name="req_type" value="routine" id="rt_routine" checked
                            style="accent-color:#4f46e5;"> Routine</label>
                    <label style="display:inline-flex;align-items:center;gap:3px;font-size:8.5pt;cursor:pointer;"><input
                            type="radio" name="req_type" value="stat" id="rt_stat" style="accent-color:#f59e0b;">
                        STAT</label>
                </div>
            </div>
            <div class="fg"><span class="fl">Clinical Diagnosis</span><input type="text" id="f_diag" class="fi"
                    value="{{ $clinicalDiagnosis ?? '' }}"></div>
        </div>

        <div class="divider"></div>

        {{-- ── REQUESTING PHYSICIAN — read-only (auto-filled from doctor session) ── --}}
        ...
        <div class="tests-grid">
            <div style="display:flex;flex-direction:column;gap:5px;">
                <div class="test-section">
                    <div class="test-sec-head" style="background:#dbeafe;color:#1e40af;"><span class="test-sec-dot"
                            style="background:#3b82f6;"></span>Hematology</div>
                    <div id="sec-hema"></div>
                </div>
                <div class="test-section">
                    <div class="test-sec-head" style="background:#f3f4f6;color:#374151;"><span class="test-sec-dot"
                            style="background:#6b7280;"></span>Blood Typing</div>
                    <div class="test-item" onclick="toggle(this)">
                        <div class="cb"></div><span class="test-name">Blood Typing</span>
                    </div>
                    <div class="test-item" onclick="toggle(this)">
                        <div class="cb"></div><span class="test-name">Crossmatching</span>
                    </div>
                </div>
                <div class="test-section">
                    <div class="test-sec-head" style="background:#ede9fe;color:#5b21b6;"><span class="test-sec-dot"
                            style="background:#8b5cf6;"></span>Serology</div>
                    <div id="sec-serology"></div>
                </div>
            </div>
            <div style="display:flex;flex-direction:column;gap:5px;">
                <div class="test-section">
                    <div class="test-sec-head" style="background:#dcfce7;color:#166534;"><span class="test-sec-dot"
                            style="background:#22c55e;"></span>Clinical Chemistry</div>
                    <div id="sec-chem"></div>
                </div>
                <div class="test-section">
                    <div class="test-sec-head" style="background:#fee2e2;color:#991b1b;"><span class="test-sec-dot"
                            style="background:#ef4444;"></span>Lipid Profile</div>
                    <div id="sec-lipid"></div>
                </div>
                <div class="test-section">
                    <div class="test-sec-head" style="background:#fce7f3;color:#9d174d;"><span class="test-sec-dot"
                            style="background:#ec4899;"></span>Serum Electrolytes</div>
                    <div id="sec-electro"></div>
                </div>
                <div class="test-section">
                    <div class="test-sec-head" style="background:#e0f2fe;color:#0c4a6e;"><span class="test-sec-dot"
                            style="background:#0ea5e9;"></span>Renal Profile</div>
                    <div id="sec-renal"></div>
                </div>
                <div class="test-section">
                    <div class="test-sec-head" style="background:#d1fae5;color:#065f46;"><span class="test-sec-dot"
                            style="background:#10b981;"></span>HBT Profile</div>
                    <div id="sec-hbt"></div>
                </div>
            </div>
            <div style="display:flex;flex-direction:column;gap:5px;">
                <div class="test-section">
                    <div class="test-sec-head" style="background:#fef9c3;color:#854d0e;"><span class="test-sec-dot"
                            style="background:#eab308;"></span>Clinical Microscopy</div>
                    <div id="sec-micro"></div>
                </div>
                <div class="test-section">
                    <div class="test-sec-head" style="background:#ffedd5;color:#9a3412;"><span class="test-sec-dot"
                            style="background:#f97316;"></span>Microbiology</div>
                    <div id="sec-mbio"></div>
                    <div class="micro-extra"><span
                            style="font-size:7pt;color:#888;text-transform:uppercase;letter-spacing:.04em;">Specimen</span><input
                            type="text" id="f_specimen" class="micro-fi" placeholder="Specimen type"></div>
                    <div class="micro-extra"><span
                            style="font-size:7pt;color:#888;text-transform:uppercase;letter-spacing:.04em;">Antibiotics
                            / Duration</span><input type="text" id="f_antibiotics" class="micro-fi"
                            placeholder="Antibiotic name · duration"></div>
                </div>
                <div class="test-section">
                    <div class="test-sec-head" style="background:#f3f4f6;color:#374151;"><span class="test-sec-dot"
                            style="background:#9ca3af;"></span>Others (Send-Out)</div>
                    <div style="padding:5px 7px;"><textarea id="f_others"
                            style="width:100%;border:1px solid #e5e7eb;border-radius:3px;font-size:8pt;padding:3px 5px;resize:none;height:48px;outline:none;font-family:inherit;"
                            placeholder="Other tests / remarks…"></textarea></div>
                </div>
            </div>
        </div>
        ...
        {{-- Date → auto-filled from PHP (shown as text)
        Request Received, Specimen Collected, Test Started, Test Done
        → filled later by the tech; shown as blank lines on this form --}}
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr;gap:8px;margin-bottom:8px;">
            ...
            <div class="fg">
                <span class="fl">Specimen Collected</span>
                <div class="fi-ro">&nbsp;</div>
            </div>
            <div class="fg">
                <span class="fl">Test Started</span>
                <div class="fi-ro">&nbsp;</div>
            </div>
            <div class="fg">
                <span class="fl">Test Done</span>
                <div class="fi-ro">&nbsp;</div>
            </div>
        </div>
    ...
    <script>
        ...
        Object.entries(TESTS).forEach(([id, tests]) => {
            ...
            tests.forEach(name => { const d = document.createElement('div'); d.className = 'test-item'; d.onclick = function () { toggle(this); }; d.innerHTML = `<div class="cb"></div><span class="test-name">${name}</span>`; c.appendChild(d); });
        });
        function toggle(el) { el.classList.toggle('checked'); }
        function getSelectedTests() { return Array.from(document.querySelectorAll('.test-item.checked .test-name')).map(el => el.textContent.trim()); }

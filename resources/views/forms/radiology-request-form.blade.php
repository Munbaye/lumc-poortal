{{--
    ...
    Path : resources/views/forms/radiology-request-form.blade.php
    Route: GET  /forms/radiology-request/{visit}  → radiologyRequest()
           POST /forms/radiology-request/{visit}  → radiologyRequestStore()

    Changes vs previous version:
      - Auto-generated $requestNo (RAD-YYYY-NNNNN) shown and hidden in form
      - Modality: only X-RAY, ULTRASOUND, CT SCAN (MRI and ECG removed)
      - "Requesting Physician" moved to TOP of interpretation section
      - "Radiologist Interpretation" is a large full-width textarea
      - "Submit Request" button saves via fetch() POST → logs to DB
--}}
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media screen {
            ...
            .paper { width: 8.5in; min-height: 14in; margin: 0 auto; background: #fff; box-shadow: 0 4px 28px rgba(0,0,0,.28); padding: 0.55in 0.65in; }
        }
        @media print {
            body  { background: #fff; padding: 0; }
            .paper { width: 100%; padding: 0; box-shadow: none; }
            .no-print { display: none !important; }
            input, select, textarea { border-color: #000 !important; background: transparent !important; outline: none !important; -webkit-print-color-adjust: exact; }
        }
        ...
        .interp-block { border: 1.5px solid #000; padding: 8px 10px; margin-top: 4px; }
        .interp-block .req-phys { margin-bottom: 10px; padding-bottom: 8px; border-bottom: 1px dashed #ccc; }
...
<div class="paper">
    ...
    <div class="screen-tip no-print">
        💡 Pre-filled fields can be edited. Click <strong>Submit Request</strong> to save to the system, then <strong>Print / PDF</strong> for the hardcopy.
    </div>
    ...
    {{-- ── MODALITY (X-RAY / ULTRASOUND / CT SCAN only) ────────────── --}}
    ...
    {{-- ── TOP ROW: DATE / RAD FILE NO / HOSPITAL NO / WARD ───────── --}}
    <div class="g4" style="margin-bottom:7px;">
        <div class="fg">
            <span class="fl">Date</span>
            <input type="date" id="f_date" class="fi" value="{{ $today ?? '' }}">
        </div>
        <div class="fg">
            <span class="fl">RAD File No.</span>
            <input type="text" id="f_rad_file" class="fi"
                   value="{{ $requestNo }}"
                   style="font-family:monospace;font-weight:bold;">
        </div>
        <div class="fg">
            <span class="fl">Hospital No. (Case No.)</span>
            <input type="text" id="f_hospital_no" class="fi" value="{{ $hospitalNo ?? '' }}">
        </div>
        <div class="fg">
            <span class="fl">Service / Ward</span>
            <input type="text" id="f_ward" class="fi" value="{{ $ward ?? '' }}">
        </div>
    </div>
    ...
    <div class="g3" style="margin-bottom:7px;">
        <div class="fg">
            <span class="fl">Family Name</span>
            <input type="text" id="f_family" class="fi" value="{{ $familyName ?? '' }}">
        </div>
        <div class="fg">
            <span class="fl">Given Name</span>
            <input type="text" id="f_first" class="fi" value="{{ $firstName ?? '' }}">
        </div>
        <div class="fg">
            <span class="fl">Middle Name</span>
            <input type="text" id="f_middle" class="fi" value="{{ $middleName ?? '' }}">
        </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:10px;margin-bottom:8px;">
        <div class="fg">
            ...
            <input type="text" id="f_address" class="fi" value="{{ $address ?? '' }}">
        </div>
        <div class="fg">
            <span class="fl">Date of Birth</span>
            <input type="date" id="f_dob" class="fi" value="{{ $dateOfBirth ?? '' }}">
        </div>
        <div class="fg">
            <span class="fl">Age</span>
            <input type="text" id="f_age" class="fi" value="{{ $age ?? '' }}">
        </div>
        <div class="fg">
    ...
    <div class="fg" style="margin-bottom:7px;">
        <span class="fl">Examination Desired</span>
        <textarea id="f_exam" rows="3" class="area" placeholder="Specify exam — e.g., Chest X-Ray PA, Whole Abdomen Ultrasound, CT Brain non-contrast…"></textarea>
    </div>

    {{-- ── CLINICAL DIAGNOSIS ───────────────────────────────────────── --}}
    <div class="fg" style="margin-bottom:7px;">
        <span class="fl">Clinical Diagnosis</span>
        <textarea id="f_clin_diag" rows="2" class="area">{{ $clinicalDiagnosis ?? '' }}</textarea>
    </div>

    {{-- ── PERTINENT CLINICAL FINDINGS ─────────────────────────────── --}}
    <div class="fg" style="margin-bottom:9px;">
        <span class="fl">Pertinent / Brief Clinical Findings</span>
        <textarea id="f_findings" rows="3" class="area" placeholder="Relevant history, symptoms, and clinical findings…"></textarea>
    </div>
    ...
    {{-- ── INTERPRETATION BLOCK
         Layout (top to bottom):
           1. Requesting Physician (name field + signature line)
           2. Radiologist Interpretation — FULL WIDTH big textarea
    ───────────────────────────────────────────────────────────────── --}}

    {{-- Requesting Physician — moved to TOP --}}
    <div style="margin-top:10px;margin-bottom:12px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;align-items:end;">
            <div class="fg" style="margin-bottom:0;">
                ...
                <input type="text" id="f_req_phys" class="fi"
    ...
    {{-- Radiologist Interpretation — large, full-width --}}
    <div>
        <div class="sec-label">Radiologist Interpretation / Findings</div>
        <textarea id="f_interp" rows="10" class="area"
                  style="border:1.5px solid #000;min-height:170px;"
                  placeholder="Radiologist findings and impression will be recorded here…"></textarea>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-top:6px;">
            <div>
                <div class="sig-line" style="margin-top:24px;"></div>
                <div class="sig-cap">Radiologist — Signature / PRC No.</div>
            </div>
            <div>
                <div class="sig-line" style="margin-top:24px;"></div>
                <div class="sig-cap">Date &amp; Time Reported</div>
            </div>
        </div>
    </div>
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:6px;">
        <div class="fg">
            ...
            <input type="date" id="f_date_req" class="fi" value="{{ $today ?? '' }}">
        </div>
        <div class="fg">
            <span class="fl">Request Received</span>
            <input type="time" id="f_received" class="fi">
        </div>
        <div class="fg">
            <span class="fl">Exam Done</span>
            <input type="time" id="f_exam_done" class="fi">
        </div>
        <div class="fg">
            <span class="fl">Released</span>
            <input type="time" id="f_released" class="fi">
        </div>
    </div>
...
<script>
/* ── Submit request via fetch (saves to DB without page reload) ──── */
async function submitForm() {
    ...
    const payload = {
        request_no:                 document.getElementById('f_rad_file').value.trim(),
        modality,
        source,
        ...
        radiologist_interpretation: document.getElementById('f_interp').value.trim(),
        requesting_physician:       document.getElementById('f_req_phys').value.trim(),
        date_requested:             document.getElementById('f_date_req').value,
    };

    try {
        const res = await fetch('{{ route("forms.radiology-request.store", ["visit" => $visit->id]) }}', {
            method: 'POST',
            headers: {

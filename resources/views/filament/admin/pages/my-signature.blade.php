...
.sig-page { max-width: 900px; margin: 0 auto; padding: 1.5rem 1rem 4rem; }
...
html.dark .sig-main-tabs { border-color:#334155; }
.sig-main-tab {
...
.sig-main-tab:hover { color:#374151; }
html.dark .sig-main-tab:hover { color:#e2e8f0; }
.sig-main-tab.active { color:var(--sig-accent); border-bottom-color:var(--sig-accent); }
.sig-main-tab svg { width:15px; height:15px; }
.sig-main-tab .count {
    background:var(--sig-accent10); color:var(--sig-accent);
...
html.dark .sig-stat-card { background:#1e293b; border-color:#334155; }
.sig-stat-card .num { font-size:2rem; font-weight:800; line-height:1; }
.sig-stat-card .lbl { font-size:.78rem; color:#6b7280; margin-top:.25rem; }
html.dark .sig-stat-card .lbl { color:#94a3b8; }
.sig-stat-card.green .num { color:#10b981; }
.sig-stat-card.red   .num { color:#f59e0b; }

.sig-staff-table { width:100%; border-collapse:collapse; }
.sig-staff-table th {
    text-align:left; font-size:.72rem; font-weight:700; color:#9ca3af;
    text-transform:uppercase; letter-spacing:.05em;
    padding:.6rem .875rem; border-bottom:1px solid #f3f4f6; background:#f9fafb;
}
html.dark .sig-staff-table th { background:#0f172a; border-color:#1e293b; color:#475569; }
.sig-staff-table td { padding:.7rem .875rem; border-bottom:1px solid #f9fafb; font-size:.83rem; color:#374151; }
html.dark .sig-staff-table td { border-color:#1e293b; color:#cbd5e1; }
.sig-staff-table tr:last-child td { border-bottom:none; }
.sig-staff-table tr:hover td { background:#f9fafb; }
html.dark .sig-staff-table tr:hover td { background:#0f172a; }

.sig-badge {
    display:inline-flex; align-items:center; gap:.3rem;
    padding:.2rem .65rem; border-radius:999px; font-size:.73rem; font-weight:600;
}
.sig-badge-green { background:#ecfdf5; color:#059669; border:1px solid #a7f3d0; }
.sig-badge-gray  { background:#f3f4f6; color:#9ca3af; border:1px solid #e5e7eb; }
html.dark .sig-badge-green { background:#064e3b; color:#6ee7b7; border-color:#065f46; }
html.dark .sig-badge-gray  { background:#1e293b; color:#475569; border-color:#334155; }
...
.sig-staff-table-header {
    padding:.9rem 1.25rem; border-bottom:1px solid #f3f4f6;
    display:flex; align-items:center; gap:.6rem;
}
html.dark .sig-staff-table-header { border-color:#334155; }
.sig-staff-table-header svg { width:18px; height:18px; color:#6b7280; }
...
<div class="sig-page" x-data="signaturePage()" x-init="init()">
    ...
    <div class="sig-header">
        ...
        <div>
            ...
            <p>Manage your own signature and view the signature status of all staff members.</p>
        </div>
    </div>
    ...
    <div class="sig-main-tabs">
        ...
        <button class="sig-main-tab" :class="{active: mainTab==='staff'}" @click="switchMainTab('staff')">
            <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
            Staff Signatures
            <span class="count">{{ $staffWithSig }}/{{ $staffSignatures->count() }}</span>
        </button>
    </div>
    ...
    ══════════════════════════════════════════════════════════ --}}
    <div x-show="mainTab==='staff'" x-cloak class="sig-staff-content">

        <div class="sig-overview-stats">
            <div class="sig-stat-card green">
                <div class="num">{{ $staffWithSig }}</div>
                <div class="lbl">Staff with signature on file</div>
            </div>
            <div class="sig-stat-card red">
                <div class="num">{{ $staffWithoutSig }}</div>
                <div class="lbl">Staff missing signature</div>
            </div>
        </div>

        <div class="sig-panel">
            <div class="sig-staff-table-header">
                <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/>
                </svg>
                <span class="title">All Staff Members</span>
                <span class="total">{{ $staffSignatures->count() }} total</span>
            </div>
            <div style="overflow-x:auto;">
                <table class="sig-staff-table">
                    <thead>
                        <tr>
                            <th>Name</th>
                            <th>Panel / Role</th>
                    ...
                    <tbody>
                        @forelse($staffSignatures as $staff)
                        <tr>
                            <td style="font-weight:600;">{{ $staff['name'] }}</td>
                            <td><span class="sig-panel-badge">{{ $staff['panel'] }}</span></td>
                            <td style="color:#6b7280;">{{ $staff['specialty'] ?? '—' }}</td>
                            <td>
                                @if($staff['has_sig'])
                                <span class="sig-badge sig-badge-green">
                                    <svg fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24" style="width:11px;height:11px;"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg>
                                    Uploaded
                                </span>
                                @else
                                <span class="sig-badge sig-badge-gray">
                                    <svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="width:11px;height:11px;"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>
                                    Not yet
                                </span>
                                @endif
                            </td>

<x-filament-panels::page>
    ...
    <style>
        ...
        .vt-table {
            width: 100%;
            min-width: 720px;
            /* prevents crushing on narrow viewports */
        ...
        .dark .vt-table tbody tr:hover td {
        ...
        .vt-table tbody tr:hover .row-arrow {
            color: #1d4ed8;
        }
    ...
    @if($patient)
        ...
        <div class="ph-header">
            ...
            <div class="ph-pills">
                ...
                @if($patient->contact_number)
                    <span class="ph-pill">
                        <x-heroicon-o-phone class="w-4 h-4 inline -mt-0.5 mr-1 text-white" />
                        {{ $patient->contact_number }}
                    </span>
                @endif
                @if($patient->has_incomplete_info)
                    <span class="ph-pill" style="background:rgba(220,38,38,.3);">
                        <x-heroicon-o-exclamation-triangle class="w-4 h-4 inline -mt-0.5 mr-1 text-red-700" />
                        Incomplete Info
                    </span>
                @endif
            </div>
            ...
            @if($visits->isEmpty())
                <div class="empty-state">
                    <div class="empty-icon">
                        <x-heroicon-o-folder-open class="w-12 h-12 text-white" />
                    </div>
                    ...
                    <tbody>
                        @foreach($visits as $visit)
                            ...
                            <tr class="{{ $isCurrent ? 'current-row' : '' }}"
                                ...
                                <td class="col-type">
                                    <span class="type-badge {{ $visit->visit_type === 'ER' ? 'type-er' : 'type-opd' }}">
                                        @if($visit->visit_type === 'ER')
                                            <x-heroicon-o-bolt class="w-4 h-4 inline -mt-0.5 mr-1" />
                                            ER
                                        @else
                                            <x-heroicon-o-document-text class="w-4 h-4 inline -mt-0.5 mr-1" />
                                            OPD
                                        @endif
                                    </span>
                                    @if($isCurrent)
                                        <span class="current-badge" style="display:inline-flex;margin-top:3px;">Active</span>
                                    @endif
                                </td>
                                ...
                                <td class="col-dx">
                                    <span class="truncate" style="max-width:150px;font-size:.78rem;color:#6b7280;"
                                        title="{{ $dx }}">
                                        {{ $dx ? \Str::limit($dx, 40) : '—' }}
                                    </span>
                                </td>
                                ...
                                <td class="col-dr">
                                    @if($hvHx?->doctor?->name)
                                        <span style="font-size:.78rem;font-weight:600;color:#374151;">Dr.
                                            {{ $hvHx->doctor->name }}</span>
                                    @else
        ...
        @if($visits->isNotEmpty())
            <p style="font-size:.7rem;color:#9ca3af;margin-top:8px;">
                ...
                &nbsp;·&nbsp;
                <x-heroicon-o-check-circle class="w-3.5 h-3.5 inline -mt-0.5 text-yellow-500" />
                = currently admitted
            </p>
        @endif

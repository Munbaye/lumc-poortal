...
<style>
/* ── Stepper ─────────────────────────────────────────────────── */
.ca-stepper { display:flex; align-items:center; margin-bottom:20px; }
.ca-step { display:flex; align-items:center; flex:1; }
.ca-step:last-child { flex:0; }
.ca-step-dot { width:32px; height:32px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:.78rem; font-weight:800; flex-shrink:0; transition:all .2s; }
.ca-step-dot.done   { background:#059669; color:#fff; cursor:pointer; }
.ca-step-dot.active { background:#1d4ed8; color:#fff; box-shadow:0 0 0 4px rgba(29,78,216,.15); }
.ca-step-dot.wait   { background:#f3f4f6; color:#9ca3af; cursor:default; }
.dark .ca-step-dot.wait { background:#374151; }
.ca-step-label { font-size:.75rem; font-weight:600; margin-left:8px; white-space:nowrap; }
.ca-step-label.done   { color:#059669; }
.ca-step-label.active { color:#1d4ed8; }
.ca-step-label.wait   { color:#9ca3af; }
.ca-step-line { flex:1; height:2px; margin:0 10px; }
.ca-step-line.done { background:#059669; }
.ca-step-line.wait { background:#e5e7eb; }
.dark .ca-step-line.wait { background:#374151; }

/* ── Patient header ──────────────────────────────────────────── */
.ca-header { background:linear-gradient(135deg,#1e3a5f,#1d4ed8); border-radius:10px; padding:14px 20px; margin-bottom:16px; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:10px; }
.ca-header-name { font-size:1rem; font-weight:800; color:#fff; }
.ca-header-case { font-family:monospace; font-size:.78rem; color:#93c5fd; margin-top:2px; }
.ca-header-pill { background:rgba(255,255,255,.15); border:1px solid rgba(255,255,255,.22); border-radius:6px; padding:4px 12px; font-size:.78rem; color:#e0f2fe; font-weight:600; }
.ca-back-link { background:rgba(255,255,255,.15); border:1px solid rgba(255,255,255,.3); color:#fff; font-size:.78rem; font-weight:600; padding:7px 14px; border-radius:6px; text-decoration:none; display:inline-flex; align-items:center; gap:5px; }
.ca-back-link:hover { background:rgba(255,255,255,.25); }

/* ── Iframe wrapper ──────────────────────────────────────────── */
.form-iframe-wrap { border:1px solid #e5e7eb; border-radius:8px; overflow:hidden; background:#fff; }
.dark .form-iframe-wrap { border-color:#374151; background:#1f2937; }
.form-iframe-wrap iframe { display:block; width:100%; border:none; }

/* ── Step hint box ───────────────────────────────────────────── */
.step-hint { font-size:.82rem; color:#6b7280; margin-bottom:12px; background:#f9fafb; border:1px solid #e5e7eb; border-radius:8px; padding:12px 16px; }
.dark .step-hint { background:#1f2937; border-color:#374151; color:#9ca3af; }

/* ── Step 4 review: form section headers ─────────────────────── */
.review-head { display:flex; align-items:center; justify-content:space-between; padding:11px 16px; background:#f9fafb; border:1px solid #e5e7eb; border-bottom:none; border-radius:8px 8px 0 0; }
.dark .review-head { background:#111827; border-color:#374151; }
.review-title { font-size:.88rem; font-weight:700; color:#111827; }
.dark .review-title { color:#f3f4f6; }
.review-meta  { font-size:.72rem; color:#6b7280; margin-top:2px; }
.review-frame-wrap { border:1px solid #e5e7eb; border-radius:0 0 8px 8px; overflow:hidden; margin-bottom:24px; }
.dark .review-frame-wrap { border-color:#374151; }

/* ── Buttons ─────────────────────────────────────────────────── */
.btn-complete { background:#059669; color:#fff; border:none; border-radius:8px; padding:12px 32px; font-size:.9rem; font-weight:700; cursor:pointer; display:inline-flex; align-items:center; gap:7px; }
.btn-complete:hover { background:#047857; }
.btn-complete:disabled { opacity:.5; cursor:not-allowed; }
.btn-back-link { background:#fff; color:#374151; border:1px solid #e5e7eb; border-radius:8px; padding:12px 22px; font-size:.88rem; font-weight:600; cursor:pointer; text-decoration:none; display:inline-flex; align-items:center; gap:5px; }
.dark .btn-back-link { background:#374151; color:#e5e7eb; border-color:#4b5563; }
.btn-back-link:hover { background:#f3f4f6; }
.btn-sm { background:none; border:1px solid #e5e7eb; border-radius:6px; padding:5px 12px; font-size:.75rem; color:#374151; cursor:pointer; text-decoration:none; display:inline-flex; align-items:center; gap:4px; }
.btn-sm:hover { background:#f3f4f6; }

/* ── Loading overlay ─────────────────────────────────────────── */
.iframe-saving-overlay { display:none; position:absolute; inset:0; background:rgba(255,255,255,.75); z-index:10; align-items:center; justify-content:center; flex-direction:column; gap:12px; border-radius:8px; }
.dark .iframe-saving-overlay { background:rgba(17,24,39,.75); }
.iframe-saving-overlay.show { display:flex; }
.spinner { width:36px; height:36px; border:4px solid #e5e7eb; border-top-color:#1d4ed8; border-radius:50%; animation:spin .7s linear infinite; }
@keyframes spin { to { transform:rotate(360deg); } }
.saving-msg { font-size:.85rem; font-weight:600; color:#374151; }
.dark .saving-msg { color:#e5e7eb; }
.iframe-container { position:relative; }

/* ── Step 4 success banner ───────────────────────────────────── */
.all-done-banner { background:#f0fdf4; border:1.5px solid #86efac; border-radius:8px; padding:14px 18px; margin-bottom:20px; }
</style>

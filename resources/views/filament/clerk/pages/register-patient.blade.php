    ...
    <div style="background:#fff;border-radius:12px;width:100%;max-width:420px;
        ...
        <div style="padding:24px;">
            <div style="margin-bottom:12px;">
                <p style="font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#6b7280;margin:0 0 5px;">Username</p>
                <div style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:11px 14px;font-family:monospace;font-size:1rem;font-weight:700;color:#111827;user-select:all;letter-spacing:.02em;">{{ $credUsername }}</div>
            </div>
            <div style="margin-bottom:20px;">
                <p style="font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#6b7280;margin:0 0 5px;">Initial Password</p>
            ...
            <button wire:click="dismissCredentialsModal" wire:loading.attr="disabled" wire:loading.class="opacity-60"
                ...
                <span wire:loading.remove wire:target="dismissCredentialsModal">I've noted the credentials — Continue</span>
...
<div style="background:linear-gradient(135deg,#1e3a5f 0%,#1d4ed8 100%);border:1px solid #1e40af;border-radius:12px;margin-bottom:24px;overflow:hidden;">
    <div style="display:flex;align-items:center;justify-content:space-between;padding:16px 24px;">
        ...
        <div style="text-align:center;flex:1;margin:0 16px;">
            <p style="color:#93c5fd;font-size:11px;letter-spacing:.1em;text-transform:uppercase;margin:0 0 2px;">Republic of the Philippines | Province of La Union</p>
            <h1 style="color:#fff;font-size:1.4rem;font-weight:700;margin:0 0 6px;text-shadow:0 1px 3px rgba(0,0,0,.4);">LA UNION MEDICAL CENTER</h1>
            <div style="display:inline-flex;align-items:center;gap:6px;padding:3px 14px;border-radius:9999px;background:rgba(255,255,255,.18);color:#e0f2fe;font-size:13px;font-weight:600;">
                @if($isUnknownMode)
...
<style>
    .lumc-select { -webkit-appearance:menulist !important; -moz-appearance:menulist !important; appearance:menulist !important; background-image:none !important; }
    .lumc-btn-primary { display:inline-flex;align-items:center;gap:8px;background:linear-gradient(135deg,#1e3a5f,#1d4ed8);color:#fff;border:none;padding:11px 28px;border-radius:9px;font-size:.875rem;font-weight:700;cursor:pointer;letter-spacing:.02em;box-shadow:0 2px 8px rgba(29,78,216,.25);transition:all .18s; }
    .lumc-btn-primary:hover { background:linear-gradient(135deg,#1e40af,#2563eb);box-shadow:0 4px 16px rgba(29,78,216,.35);transform:translateY(-1px); }
    .lumc-btn-danger { display:inline-flex;align-items:center;gap:8px;background:linear-gradient(135deg,#b91c1c,#dc2626);color:#fff;border:none;padding:11px 28px;border-radius:9px;font-size:.875rem;font-weight:700;cursor:pointer;letter-spacing:.02em;box-shadow:0 2px 8px rgba(220,38,38,.25);transition:all .18s; }
    .lumc-btn-danger:hover { background:linear-gradient(135deg,#991b1b,#b91c1c);box-shadow:0 4px 16px rgba(220,38,38,.35);transform:translateY(-1px); }
    .lumc-btn-secondary { display:inline-flex;align-items:center;gap:8px;background:#fff;color:#4b5563;border:1.5px solid #d1d5db;padding:11px 24px;border-radius:9px;font-size:.875rem;font-weight:600;cursor:pointer;letter-spacing:.01em;transition:all .18s; }
    .lumc-btn-secondary:hover { background:#f3f4f6;border-color:#9ca3af;color:#111827;transform:translateY(-1px); }
    .lumc-input { width:100%;border-radius:8px;padding:10px 12px;font-size:.875rem;border:1px solid #d1d5db;background:#fff;color:#111827;outline:none;box-sizing:border-box; }
    ...
    .lumc-section { background:#fff;border:1px solid #e5e7eb;border-radius:12px;padding:24px;margin-bottom:16px; }
</style>

{{-- ══ UNKNOWN MODE ══ --}}
@if($isUnknownMode)
<div style="background:#fff;border:2px solid #dc2626;border-radius:12px;padding:24px;margin-bottom:20px;">
    <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:20px;">
        <div>
            <h2 style="font-size:1rem;font-weight:700;color:#dc2626;margin:0 0 4px;">🚨 Unknown Patient Registration</h2>
            <p style="font-size:.82rem;color:#6b7280;margin:0;">Registered as ER. Identity can be updated later by the clerk or admin.</p>
        </div>
        <button wire:click="cancelUnknownMode" class="lumc-btn-secondary">← Cancel</button>
    </div>
    <div style="margin-bottom:16px;">
        <label class="lumc-label {{ $errors->has('unknownFormData.chief_complaint') ? 'lumc-label-error' : '' }}">Chief Complaint / Reason for ER Visit *</label>
        ...
        @error('unknownFormData.chief_complaint')<p class="lumc-error">⚠️ {{ $message }}</p>@enderror
    </div>
    ...
    <button wire:click="saveUnknown" wire:loading.attr="disabled" wire:loading.class="opacity-60" class="lumc-btn-danger">
...
<div class="lumc-section">
    ...
    <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:12px;margin-bottom:16px;">
        ...
        <div>
            <label class="lumc-label">Birthday <span style="font-size:.65rem;color:#9ca3af;font-weight:400;">(±1 yr)</span></label>
    ...
    @if($hasSearched)
        @if(count($searchResults) > 0)
                ...
                <table style="width:100%;border-collapse:collapse;font-size:.82rem;">
                    <thead style="background:#1e3a5f;">
                        <tr>
                            ...
                            <th style="padding:10px 12px;text-align:left;color:#bfdbfe;font-size:.72rem;font-weight:600;white-space:nowrap;">{{ $h }}</th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr style="border-top:1px solid #f3f4f6;background:{{ $selectedPatientId==$r['id']?'#f0fdf4':($r['has_incomplete']?'#fef2f2':'#fff') }};">
                            ...
                            <td style="padding:9px 12px;">
                                <button wire:click="selectPatient({{ $r['id'] }})"
                                        style="background:#16a34a;color:#fff;border:none;padding:6px 12px;border-radius:6px;font-size:.75rem;font-weight:600;cursor:pointer;white-space:nowrap;"
                                        onmouseover="this.style.opacity='.85'" onmouseout="this.style.opacity='1'">✓ Select</button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
            ...
            <p style="font-size:.75rem;color:#6b7280;margin-top:8px;">{{ count($searchResults) }} result(s) found — includes fuzzy/typo matches.</p>
        @else
            <div style="background:#fefce8;border:1px solid #fde047;border-radius:10px;padding:16px;">
                <p style="font-weight:700;color:#854d0e;margin:0 0 4px;">⚠️ No similar patients found for "<strong>{{ $searchFamilyName }}</strong>"</p>
                ...
                @if($confirmNoMatch)
                    <button wire:click="showNewPatientForm" class="lumc-btn-primary">+ Create New Patient Record</button>
                @endif
            </div>
        @endif
    @else
...
<div class="lumc-section">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:20px;">
        ...
        <div>
            ...
            @error('formData.sex')<p class="lumc-error">⚠️ {{ $message }}</p>@enderror
        </div>
        <div>
        ...
        <div>
            ...
            <input type="text" wire:model="formData.contact_number" placeholder="09XX-XXX-XXXX" class="lumc-input">
        </div>
    </div>

    {{-- Address --}}
    <div style="margin-bottom:20px;">
        <label class="lumc-label {{ $errors->has('formData.address') ? 'lumc-label-error' : '' }}">Complete Address *</label>
        <textarea wire:model="formData.address" rows="2" placeholder="e.g., Brgy. Poblacion, Agoo, La Union"
                  class="lumc-input {{ $errors->has('formData.address') ? 'lumc-input-error' : '' }}" style="resize:vertical;"></textarea>
        @error('formData.address')<p class="lumc-error">⚠️ {{ $message }}</p>@enderror
    </div>

    {{-- Optional --}}
    <p style="font-size:.7rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#9ca3af;margin:0 0 12px;">Optional Information</p>
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:20px;">
        <div>
    ...
    {{-- NOTE: ER Details section (brought_by, condition_on_arrival) intentionally removed.
         These fields are now collected on the official ER Record form (ER-001) during
         Complete Admission, matching the hospital's actual paper workflow. --}}

    {{-- Chief Complaint --}}
    <div style="margin-bottom:20px;">
        <label class="lumc-label {{ $errors->has('formData.chief_complaint') ? 'lumc-label-error' : '' }}">Chief Complaint *</label>
        <textarea wire:model="formData.chief_complaint" rows="2"
                  ...
                  style="resize:vertical;"></textarea>
        @error('formData.chief_complaint')<p class="lumc-error">⚠️ {{ $message }}</p>@enderror
    </div>
    ...
    <div style="background:#fef2f2;border:2px solid #fca5a5;border-radius:10px;padding:16px;margin-bottom:24px;">
        <div style="display:flex;align-items:flex-start;gap:10px;">
            <input type="checkbox" id="hasIncompleteInfo" wire:model.live="formData.has_incomplete_info"
                   style="width:16px;height:16px;margin-top:2px;cursor:pointer;accent-color:#dc2626;flex-shrink:0;">
            <div>
                <label for="hasIncompleteInfo" style="font-size:.85rem;font-weight:700;cursor:pointer;color:#dc2626;display:block;margin-bottom:3px;">⚠️ Patient has missing or incomplete information</label>
                <p style="font-size:.78rem;color:#9ca3af;margin:0;">When checked, this patient's name shows in <strong style="color:#dc2626;">red</strong> across all panels until resolved.</p>
            </div>
        </div>
        ...
        <div style="margin-top:10px;padding:8px 12px;background:#fff;border-radius:6px;border:1px solid #fca5a5;">
            <p style="font-size:.78rem;color:#dc2626;font-weight:600;margin:0;">🔴 Patient will be flagged as having incomplete information.</p>
        </div>
        @endif
    </div>

    {{-- Submit --}}
    <button wire:click="save" wire:loading.attr="disabled" wire:loading.class="opacity-60" class="lumc-btn-primary">
